...
const CONTRACT_METADATA: Record<
	string,
	{ title: string; description: string; functions: string[] }
> = {
	HederaGameLaunchpad: {
	...
	FaucetManager: {
		title: 'FaucetManager V2',
		description:
			'HBAR to HPLAY token swap with direct minting (V2 - Simplified).',
		functions: [
			'swapHBARforHPLAY() payable returns (uint256)',
			'updateSwapRate(uint256 newRate)',
			'setDailyLimit(uint256 newLimit)',
			'setEnabled(bool enabled)',
			'getUserInfo(address user) view returns (tuple)',
			'getSwapRate() view returns (tuple)',
			'getRemainingLimit(address user) view returns (uint256)',
			'isNewDay(uint256 lastTimestamp) view returns (bool)',
			'pause()',
			'unpause()',
		],
	},
...
export async function GET() {
	// Compose addresses
	const addresses = {
		HederaGameLaunchpad: envOrFallback(
			'HEDERA_GAME_LAUNCHPAD_ADDRESS',
			CONTRACT_ADDRESSES.HederaGameLaunchpad
		),
		GameRegistry: envOrFallback(
			'GAME_REGISTRY_ADDRESS',
			CONTRACT_ADDRESSES.GameRegistry
		),
		ResultVerifier: envOrFallback(
			'RESULT_VERIFIER_ADDRESS',
			CONTRACT_ADDRESSES.ResultVerifier
		),
		PlayerSBT: envOrFallback(
			'PLAYER_SBT_ADDRESS',
			CONTRACT_ADDRESSES.PlayerSBT
		),
		NFTManager: envOrFallback(
			'NFT_MANAGER_ADDRESS',
			CONTRACT_ADDRESSES.NFTManager
		),
		TokenEconomy: envOrFallback(
			'TOKEN_ECONOMY_ADDRESS',
			CONTRACT_ADDRESSES.TokenEconomy
		),
		FaucetManager: envOrFallback(
			'FAUCET_MANAGER_ADDRESS',
			CONTRACT_ADDRESSES.FaucetManager
		),
		LotteryPool: envOrFallback(
			'LOTTERY_POOL_ADDRESS',
			CONTRACT_ADDRESSES.LotteryPool
		),
	};
