...
// IPFS configuration for Filebase (client-side only)
const IPFS_CONFIG = getIPFSClientConfig();

// Create IPFS client (only in browser)
let ipfs: any = null;

// Lazy load IPFS client
async function getIPFSClient() {
  if (typeof window === 'undefined') {
    throw new Error('IPFS client not available on server');
  }
  ...
  if (!ipfs) {
    try {
      ...
      if (create && typeof create === 'function') {
        ...
        throw new Error('IPFS client create function not available');
      }
    } catch (error) {
